...
<head>
    ...
    <title>La Misión · Evento de Minecraft</title>
...
<body>
    <div class="wrapper">
        ...
        <div class="grid">
            ...
            <div class="side-panel">
                ...
                <div class="footnote">
                    <i class="fas fa-book-skull"></i> <strong>📖 Libros del jugador</strong> (solo vivos).<br>
                    <span style="background: #0a5555; padding: 4px 16px; border-radius: 30px;">✅ Vivo · Nadie le mató</span><br>
                    <span style="background: #5a4a3a; padding: 4px 16px; border-radius: 30px;">⏸️ Inactividad · Abandonó</span><br>
                    <span style="background: #632c2c; padding: 4px 16px; border-radius: 30px;">💀 Muerto · Asesinado</span><br>
                    <span style="background: #3a4f44; padding: 4px 16px; border-radius: 30px; margin-top:8px; display:inline-block;">
                        <i class="fas fa-user-secret"></i> Jugador original: quien debía realizar la misión
                    </span>
                </div>
            </div>
        </div>
    ...
    <script>
        (function(){
            ...
            // - Bulltran: 0 libros (VIVO) 🟡
            // - iPlayerBeta: 1 libro (VIVO) 🟡
            // - Tarrk: 0 libros (VIVO) 🟡
            // - Jatze: 5 libros (VIVO) 🟡
            // - Marcos131: inactividad (abandonó) 🟤
            // - Avylon: inactividad (abandonó) 🟤
            // - Nico_v: inactividad (abandonó) 🟤
            // - Nicolasfrdz: muerto por RayHZ 🔴
            // - Marcoslion: muerto por iPlayerBeta 🔴
            // - Pinpilla: muerto por Jatze 🔴
            // - Biancu: muerto por Jatze 🔴
            // - RayHZ: muerto por RayHZ (suicidio) 🔴
            // - Juanma1307: muerto por RayHZ 🔴
            // -------------------------------------------------------------
            const misiones = [
                ...
                { original: "Tarrk", forma: "Hazle un tour privado por todas las canciones que has realizado (/warp Toy Story y /warp Sirenita) y reprodúcelas al menos 3 veces cada una", objetivo: "iPlayerBeta", asesinoFinal: "0", libros: "1" },
                { original: "iPlayerBeta", forma: "Consigue que te regale exactamente 32 flores de cualquier tipo. (si te da más, se las devuelves)", objetivo: "Marcoslion", asesinoFinal: "iPlayerBeta", libros: "-" },
                { original: "Marcoslion", forma: "Consigue que vaya hacia ti para aparear dos vacas y luego le cantas (o escribes por chat global) la canción de La Vaca Lola ♫", objetivo: "Nico_v", asesinoFinal: "inactividad", libros: "-" },
                { original: "Nico_v", forma: "Hazle un tour por tu casa (Si no tienes casa, que él te haga el tour en la suya) y después le das carne de zombie como regalo y tiene que quedarsela", objetivo: "Avylon", asesinoFinal: "inactividad", libros: "-" },
                { original: "Avylon", forma: "Pídele como favor que te proteja durante 1 noche en exploración (tiene que haber mobs)", objetivo: "Jatze", asesinoFinal: "0", libros: "5" },
                { original: "Jatze", forma: "Consigue que te ayude a hacer un columpio en un árbol de cerezo.", objetivo: "Pinpilla", asesinoFinal: "Jatze", libros: "-" },
                { original: "Pinpilla", forma: "Pídele que te fabrique un set de armadura de cobre (¡tiene que ser de cobre!) y que te lo entregue.", objetivo: "Biancu", asesinoFinal: "Jatze", libros: "-" },
                { original: "Biancu", forma: "Ingéniatelas para que se coma un Pez Globo", objetivo: "RayHZ", asesinoFinal: "Jatze", libros: "-" },
            ...
            function drawCanvas() {
                ...
                // ---- FLECHAS NARANJA: JUGADOR ORIGINAL → OBJETIVO ----
                ctx.shadowBlur = 9;
                ...
                // ---- FLECHAS ROJAS EXTERIORES: ASESINO FINAL → VÍCTIMA (SOLO JUGADORES) ----
                ...
                // ---- PUNTOS DE LOS OBJETIVOS (COLOR SEGÚN ESTADO) ----
                objetivosUnicos.forEach(n => {
                    ...
                    if (vivosSet.has(n)) {
                        ...
                        puntoColor = "#b78c5a"; // Marrón inactivo
                        puntoBorde = "#8b5a2b";
                ...
                misiones.forEach(m => {
                    if (m.asesinoFinal === "0") {
                        ...
                        if (libros !== undefined && positions[objetivoNombre]) {
                            ...
                            let libroText = `📖 ${libros}`;
                            if (libros === "1") libroText = `📖 1 libro`;
                            else if (libros === "0") libroText = `📖 0 libros`;
                            else libroText = `📖 ${libros} libros`;
            ...
            function generarPanel() {
                ...
                misiones.forEach(m => {
                    ...
                    if (asesinoFinal === "0") {
                        asesinoDisplay = `<span class="alive-tag"><i class="fas fa-heart"></i> Vivo · Nadie le mató</span>`;
                    } else if (asesinoFinal === "inactividad") {
                        asesinoDisplay = `<span class="inactive-tag"><i class="fas fa-pause-circle"></i> Inactividad · Abandonó</span>`;
                    } else {
                    ...
                    html += `<div class="target-card">
                                ...
                                <div class="forma-mision">
                                    ...
                                    <i class="fas fa-dice-d6" style="margin-top:6px;"></i> Misión: ${m.forma}
                                </div>
